...
class MineMeldRunConfigTests(unittest.TestCase):
    def test_defaults_from_file(self):
        ...
        with mock.patch.dict(os.environ, {
                minemeld.run.config.FABRIC_NUM_CONNS_ENV: '50',
                minemeld.run.config.MGMTBUS_NUM_CONNS_ENV: '10'
                }):
        ...
        self.assertEqual(
            ...
            {'num_connections': 50, 'priority': minemeld.run.config.gevent_core.MINPRI}
        )
        self.assertEqual(config.mgmtbus['transport']['class'], 'ZMQRedis')
        self.assertEqual(
            config.mgmtbus['transport']['config'],
            {'num_connections': 10, 'priority': minemeld.run.config.gevent_core.MAXPRI}
        )
