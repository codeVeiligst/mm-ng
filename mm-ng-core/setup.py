...
def _native_extensions_enabled():
    value = os.environ.get('MM_NG_SKIP_NATIVE', '')
...
if _native_extensions_enabled():
    try:
        ...
        # this is for source-only installs where native extensions are skipped
...
setup(
    name='mm-ng-core',
    version=__version__,
    url='https://github.com/mm-ng/mm-ng-core',
    author='mm-ng community',
    description='MineMeld-compatible indicator processing core runtime',
    classifiers=[
    ...
    ext_modules=cythonize([GDNS]) if _native_extensions_enabled() else [],
    entry_points={
        'console_scripts': [
            'mm-run = minemeld.run.launcher:main',
            'mm-api = minemeld.flask.main:main',
