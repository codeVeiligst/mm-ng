...
def _dist_name(dist):
    result = getattr(dist, 'project_name', None)
...
def _dist_has_metadata(dist, metadata_name):
    if hasattr(dist, 'has_metadata'):
        return dist.has_metadata(metadata_name)

    return dist.read_text(metadata_name) is not None


def _dist_get_metadata(dist, metadata_name):
    if hasattr(dist, 'get_metadata'):
        return dist.get_metadata(metadata_name)

    return dist.read_text(metadata_name)


def _egg_link_path(dist):
    project_name = _dist_name(dist)
...
def _read_entry_points(ep_contents):
    if isinstance(ep_contents, bytes):
        ep_contents = ep_contents.decode('utf-8')

    parser = ConfigParser()
    parser.optionxform = str
    parser.read_string(ep_contents)

    ep_map = {}
    for section in parser.sections():
        ep_map[section] = {}
        for epname, ep in parser.items(section):
            ep_map[section][epname] = ep.strip()
...
def _activated_extensions():
    ...
    for epgroup in epgroups:
        for _, epvalue in minemeld.loader.map(epgroup).items():
            dist = epvalue.ep.dist
            dist_name = _dist_name(dist)
            if dist_name in ('minemeld-core', 'mm-ng-core'):
                continue

            location = 'site-packages'
            egg_link = _egg_link_path(dist)
            ...
            elif _dist_has_metadata(dist, 'METADATA'):
                dist_metadata = _read_metadata(
                    _dist_get_metadata(dist, 'METADATA')
                )
                for k in list(metadata.keys()):
                    if k in METADATA_MAP and METADATA_MAP[k] in dist_metadata:
                        metadata[k] = dist_metadata[METADATA_MAP[k]]

                if _dist_has_metadata(dist, 'entry_points.txt'):
                    metadata['entry_points'] = _read_entry_points(
                        _dist_get_metadata(dist, 'entry_points.txt')
                    )

            activated_extensions[dist_name] = ActivatedExtension(
                location=location,
                **metadata
            )
