...
class MgmtbusMaster(object):
    ...
    def _store_fallback_metrics(self, answers):
        minemeld.metricsstore.record_samples(
            self.SR,
            answers,
    ...
    def _status_loop(self):
        ...
        them to collected.
        """
        collectd_enabled = not _env_bool('MM_DISABLE_COLLECTD', False)
        ...
        while True:
            ...
            if success is None:
                ...
                if collectd_enabled:
                    ...
                    try:
                        self._store_fallback_metrics(result['answers'])
                    except:
                        LOG.exception('Exception storing fallback metrics')
