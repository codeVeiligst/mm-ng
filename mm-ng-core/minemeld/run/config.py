...
import yaml
try:
    import gevent.core as gevent_core
except ImportError:
    class _GeventCoreFallback(object):
        MINPRI = -2
        MAXPRI = 2

    gevent_core = _GeventCoreFallback()
...
def validate_config(config):
    ...
    for n in list(nodes.keys()):
        if re.match(r'^[a-zA-Z0-9_-]+$', n) is None:
