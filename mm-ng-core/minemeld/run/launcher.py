...
def _install_signal_handler(signum, handler):
    signal_handler = getattr(gevent, 'signal_handler', None)
    if signal_handler is not None:
        return signal_handler(signum, handler)

    gevent_signal = getattr(gevent, 'signal', None)
    if callable(gevent_signal):
        return gevent_signal(signum, handler)

    return signal.signal(signum, lambda *_: handler())
...
def _run_chassis(fabricconfig, mgmtbusconfig, fts):
    try:
        ...
        _install_signal_handler(signal.SIGUSR1, c.stop)
...
def main():
    ...
    log_file = os.environ.get('MM_ENGINE_LOG_FILE', None)
    ...
    _install_signal_handler(signal.SIGINT, _sigint_handler)
    _install_signal_handler(signal.SIGTERM, _sigterm_handler)
    ...
    try:
        ...
        chassis_ready_timeout = int(os.environ.get(
            'MM_CHASSIS_READY_TIMEOUT',
            60
        ))
        mbusmaster.start()
        mbusmaster.wait_for_chassis(timeout=chassis_ready_timeout)
