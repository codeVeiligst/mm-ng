...
def get_system_status():
    ...
    if data_path is None:
        data_path = config.get('MM_RUNTIME_CONFIG_DIR', None)
    if data_path is None:
        return jsonify(error={'message': 'MINEMELD_LOCAL_PATH not set'}), 500
...
def _clean_local_backup(local_backup_file, g):
    ...
    timer = threading.Timer(300, _safe_remove, args=(local_backup_file,))
    timer.daemon = True
    timer.start()
...
def generate_local_backup():
    ...
    sevenz_path = _runtime_config('MINEMELD_7Z_PATH', None)
    ...
    proto_path = _runtime_config('MINEMELD_LOCAL_PROTOTYPE_PATH', None)
    if proto_path is not None:
        args.append(proto_path)
    certs_path = _runtime_config('MINEMELD_LOCAL_CERTS_PATH', None)
...
def restore_local_backup(backup_id):
    restore_path = _runtime_config('MINEMELD_RESTORE_PATH', None)
    ...
    try:
        ...
        if restore_prototypes:
            p = _runtime_config('MINEMELD_LOCAL_PROTOTYPE_PATH', None)
            if p is None:
        ...
        if restore_certificates:
            p = _runtime_config('MINEMELD_LOCAL_CERTS_PATH', None)
            if p is None:
