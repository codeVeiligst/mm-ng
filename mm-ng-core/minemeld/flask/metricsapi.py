...
def _metrics_available():
    return rrdtool is not None and os.path.isdir(RRD_PATH)
...
def _fetch_fallback_metric(metric, type_=None, dt=86400, r=1800):
    return minemeld.metricsstore.fetch_metric(
        SR,
        metric,
        dt=dt,
        r=r,
...
def get_metrics():
    ...
    return jsonify(result=minemeld.metricsstore.list_metrics(SR))
...
def get_node_type_metrics(nodetype):
    ...
    if not _metrics_available():
        metrics = minemeld.metricsstore.list_metrics(SR, prefix='minemeld.'+nodetype+'.')
        result = []
        for m in metrics:
            v = _fetch_fallback_metric(m, dt=dt, r=resolution, type_=type_)
            _, _, mname = m.split('.', 2)
            result.append({
                'metric': mname,
                'values': v
...
def get_global_metrics():
    ...
    if not _metrics_available():
        metrics = minemeld.metricsstore.list_metrics(SR, prefix='minemeld.')
...
def get_node_metrics(node):
    ...
    if not _metrics_available():
        ...
        metrics = minemeld.metricsstore.list_metrics(SR, prefix=node+'.')
...
def get_metric(node, metric):
    ...
    if not _metrics_available():
        ...
        values = _fetch_fallback_metric(metric_name, dt=dt, r=resolution, type_=type_)
