...
class Sns(object):
    ...
    def _send_message(self, kvmessage):
        ...
        try:
            ...
            LOG.warning('Failure sending the message to the sns cloud provider: %s', e)
            return False
...
def init_app():
    ...
    global SNS_API_URL
    global SNS_ENABLED
    SNS_API_URL = _runtime_config('SNS_URL', 'https://minemeld-notifications.panw.io/0.9/')
    SNS_ENABLED = _runtime_config('SNS_ENABLED', False)
    UUID_FILENAME = _runtime_config('UUID_FILE', 'uu.id4')
    if not SNS_ENABLED:
        return
