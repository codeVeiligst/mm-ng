...
def _runtime_config(key, default=None):
...
def _run_command(args, timeout=None):
    try:
        ...
        LOG.error('Timeout running {}: {}'.format(args, e))
        return e


def _get_extensions():
    library_directory = _runtime_config('MINEMELD_LOCAL_LIBRARY_PATH', None)
...
def _patch_legacy_extension_source(ext_path):
    if ext_path.endswith('.whl'):
        return

    setup_path = os.path.join(ext_path, 'setup.py')
    ...
    patched_contents = setup_contents.replace('.iteritems()', '.items()')
    if patched_contents == setup_contents:
        return

    LOG.info('Patching legacy Python 2 setup.py syntax in {}'.format(setup_path))
    with open(setup_path, 'w') as f:
        f.write(patched_contents)


def _build_activate_args(ext_path):
    pip_path = _runtime_config('MINEMELD_PIP_PATH', None)
    if pip_path is None:
    ...
    _patch_legacy_extension_source(ext_path)
...
def get_git_refs():
    ...
    git_path = _runtime_config('MINEMELD_GIT_PATH', None)
    if git_path is None:
        return jsonify(error={'message': 'MINEMELD_GIT_PATH not set'}), 500

    git_args = [git_path, 'ls-remote', '-t', '-h', git_endpoint]

    git_process = _run_command(git_args, timeout=20.0)
    if isinstance(git_process, subprocess.TimeoutExpired):
        return jsonify(error={'message': 'Timeout accessing git repo'}), 400

    git_stdout = git_process.stdout.decode('utf-8', 'replace')
    git_stderr = git_process.stderr.decode('utf-8', 'replace')
