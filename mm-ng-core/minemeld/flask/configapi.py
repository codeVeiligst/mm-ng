...
def _commit_config(version):
    ...
    for n in range(config_info['next_node_id']):
        ...
        if node['name'] in newconfig['nodes']:
...
def _config_info():
    ...
    changed = _to_text(SR.hget(REDIS_KEY_CONFIG, 'changed')) == "1"
    next_node_id = int(_to_text(SR.hget(REDIS_KEY_CONFIG, 'next_node_id')))
...
def _validate_node_body(nodebody):
    if not isinstance(nodebody, dict):
        raise ValueError('node body must be an object')

    if nodebody.get('version') is None:
        raise ValueError('version required')

    name = nodebody.get('name')
    if not isinstance(name, str) or len(name.strip()) == 0:
        raise ValueError('node name required')
    if re.match(r'^[A-Za-z0-9][A-Za-z0-9_-]*$', name.strip()) is None:
        raise ValueError('node name is invalid')

    properties = nodebody.get('properties')
    if not isinstance(properties, dict):
        raise ValueError('node properties required')

    prototype = properties.get('prototype')
    if not isinstance(prototype, str) or len(prototype.strip()) == 0:
        raise ValueError('node prototype required')
    ...
    for input_name in inputs:
        if not isinstance(input_name, str) or len(input_name.strip()) == 0:
            raise ValueError('node inputs must contain strings')

    output = properties.get('output', False)
    if not isinstance(output, bool):
        raise ValueError('node output must be boolean')
...
def _candidate_config_with_node(nodebody, info):
    newconfig = {}
    ...
    newconfig['nodes'] = {}
    for n in range(info['next_node_id']):
        node = _get_stanza('node%d' % n)
        if node is None:
            continue
        ...
        if 'properties' not in node:
            raise ValueError('no properties for node %s' % node['name'])
        newconfig['nodes'][node['name']] = copy.deepcopy(node['properties'])

    if nodebody['name'] in newconfig['nodes']:
        raise ValueError('duplicate node name - %s' % nodebody['name'])
    newconfig['nodes'][nodebody['name']] = copy.deepcopy(nodebody['properties'])

    return newconfig


def _validate_candidate_config(newconfig):
    temp_config = minemeld.run.config.MineMeldConfig.from_dict(
        copy.deepcopy(newconfig)
    )
    valid = minemeld.run.config.resolve_prototypes(temp_config)
    if not valid:
        raise ValueError('Error resolving prototypes')
...
def _create_node(nodebody):
    ...
    _validate_node_body(nodebody)

    version = nodebody.pop('version', None)
    if version != info['version']:
        raise VersionMismatchError('version mismatch')

    _validate_candidate_config(_candidate_config_with_node(nodebody, info))
...
@BLUEPRINT.route('/reload', methods=['GET'], read_write=True)
...
@BLUEPRINT.route('/node', methods=['POST'], read_write=True)
def create_node():
    try:
        body = request.get_json()
    except Exception as e:
...
@BLUEPRINT.route('/node/<nodenum>', methods=['PUT'], read_write=True)
...
@BLUEPRINT.route('/node/<nodenum>', methods=['DELETE'], read_write=True)
...
def init_app(app):
    before_first_request = getattr(app, 'before_first_request', None)
    if before_first_request is not None:
        before_first_request(_init_config)
        return
