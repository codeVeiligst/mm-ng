...
class Table(object):
    ...
    def get_custom_metadata(self):
        ...
        if cmetadata is None:
            return None
        return ujson.loads(_to_text(cmetadata))
    ...
    def set_custom_metadata(self, metadata=None):
        ...
        cmetadata = _to_bytes(ujson.dumps(metadata))
    ...
    def get(self, key):
        ...
        # skip version
        return ujson.loads(_to_text(value[8:]))
    ...
    def create_index(self, attribute):
        ...
        batch = self.db.write_batch()
        batch.put(struct.pack("BBB", 0, 1, idxid), _to_bytes(attribute))
        batch.write()
    ...
    def put(self, key, value):
        ...
        batch = self.db.write_batch()
        batch.put(ikey, struct.pack(">Q", cversion)+_to_bytes(ujson.dumps(value)))
