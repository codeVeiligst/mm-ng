...
def _rpc_socket_name(name):
    if name.startswith('@'):
        return name[1:]
    return name


def _rpc_socket_path(name):
    return '/var/run/minemeld/{}:rpc'.format(_rpc_socket_name(name))


def _rpc_socket_address(name):
    return 'ipc://{}'.format(_rpc_socket_path(name))
...
class ZMQRpcServerChannel(object):
    ...
    def _send_result(self, reply_to, id_, result=None, error=None):
        ...
        if self.fanout is not None:
            ...
            self.socket.send_multipart([_to_bytes(reply_to), b'', _json_bytes(ans)])
    ...
    def run(self):
        ...
        while True:
            ...
            if self.fanout is not None:
                reply_to, body = toks
                reply_to = '{}:reply'.format(_to_text(reply_to))
            else:
                reply_to, _, body = toks

            body = json.loads(_to_text(body))
    ...
    def connect(self, context):
        ...
        if self.fanout is not None:
            ...
            self.socket_path = _rpc_socket_path(self.name)
            _safe_unlink(self.socket_path)
            address = _rpc_socket_address(self.name)
...
class ZMQSubChannel(object):
    ...
    def connect(self, context):
        ...
        self.socket = self.context.socket(zmq.SUB)
        self.socket_path = '/var/run/minemeld/{}'.format(self.topic)
        _safe_unlink(self.socket_path)
        self.socket.bind('ipc://{}'.format(self.socket_path))
        self.socket.setsockopt(zmq.SUBSCRIBE, b'')  # set the filter to empty to recv all messages
...
class ZMQRedis(object):
    ...
    def send_rpc(self, dest, method, params,
        ...
        address = _rpc_socket_address(dest)
