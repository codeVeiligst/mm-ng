...
class Table(object):
    ...
    def put(self, key, value):
        ...
        new_max_counter_b = _to_bytes(new_max_counter)

        batch = self.db.write_batch()
        batch.put(key + new_max_counter_b, value)
        batch.put(TABLE_MAX_COUNTER_KEY, new_max_counter_b)
        batch.write()

    def backwards_iterator(self, timestamp, counter):
        return self.db.iterator(
            start=START_KEY,
            stop=_to_bytes('%016x%016x' % (timestamp, counter)),
            include_start=False,
            include_stop=True,
            reverse=True
        )
