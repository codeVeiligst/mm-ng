...
def _build_graph(config):
    ...
    for nodename, nodevalue in config.nodes.items():
        ...
        for input_node in inputs:
            graph.setdefault(input_node, set()).add(nodename)
            graph[nodename].add(input_node)
...
def _plan_subgraph(sg, config, state_info):
    ...
    LOG.info('planning for subgraph {!r}'.format(sg))
    plan = {}
...
def plan(config, state_info):
    ...
    for component in _weakly_connected_components(graph):
        plan.update(_plan_subgraph(component, config, state_info))
